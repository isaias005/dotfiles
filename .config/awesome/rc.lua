...
-- Create a clock widget and an attached calendar
...
volume = lain.widget.alsa({
    settings = function()
        ...
        elseif tonumber(volume_now.level) == 0 then
            vol_icon:set_image(beautiful.widget_vol_no)
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({ "DEV", "WWW", "MEDIA", "SYS", "DOC"}, s, awful.layout.layouts[1])
    ...
    s.mywibox:setup {
        ...
        { -- Left widgets
            ...
            wibox.container.background(s.mytaglist, beautiful.bg_minimize),
            arrow_right(beautiful.bg_minimize, beautiful.bg_normal)
        },
        wibox.container.margin(s.mytasklist,10,0), -- Middle widget
        { -- Right widgets
            layout = wibox.layout.fixed.horizontal,
            arrow_left(beautiful.bg_normal, beautiful.bg_minimize),
            wibox.container.background(wibox.container.margin(wibox.widget.systray(), 2, 3), beautiful.bg_minimize),
            arrow_left(beautiful.bg_minimize, beautiful.bg_focus),
            wibox.container.background(wibox.container.margin(wibox.widget { vol_icon, volume.widget, layout = wibox.layout.align.horizontal }, 2, 3), beautiful.bg_focus),
...
globalkeys = gears.table.join(
    ...
    awful.key({ "Shift" }, "Print",
            function()
                awful.spawn.easy_async_with_shell("sleep 0.1 && scrot -s", function() end)
            end),

    -- Dmenu
    awful.key({ modkey }, "p", 
            function() 
                awful.spawn.easy_async(string.format("dmenu_run -fn '%s' -nb '%s' -sf '%s' -sb '%s' -nf '%s'", 
